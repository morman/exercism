  ...
  def self.proteins(strand : String) : Array(String)
    ...
    codons.each { |codon|
      ...
      when "UAA", "UAG", "UGA"
        ...
        raise ArgumentError.new(message = "Unexpected codon: cannot translate to protein!")
      end
    }
    ...
    if strand.size.modulo(3) != 0
      ...
      if pre_stop.size.modulo(3) != 0 
        # unexpected codon length: throw errors
         raise ArgumentError.new(message = "Unexpected codon length before STOP: cannot translate to protein!")   
      end 
    end
